...
const AppContent: React.FC = () => {
  ...
  const { items, isLoading: isItemsLoading, addItem, addItemsBatch, updateItem, removeItem, clearList } = useShoppingItems();
  ...
  const handleImportList = useCallback((listName: string, itemsToImport: ShoppingItem[]) => {
    if (window.confirm(`Tem certeza que deseja importar os ${itemsToImport.length} itens da lista "${listName}" para a sua lista atual?`)) {
      // Remove the 'id' property as Supabase will generate new ones
      const itemsWithoutId = itemsToImport.map(({ id, ...rest }) => rest);
      addItemsBatch(itemsWithoutId);
      setCurrentView('list'); // Switch back to the list view after import
    }
  }, [addItemsBatch]);
  ...
  const renderMainContent = () => {
    ...
    if (currentView === 'saved_lists') {
      return <SavedListsPage onImportList={handleImportList} />;
    }
